...
export const aiTrainingWorker = new Worker(
  ...
  async () => {
    try {
      ...
      const responseBody = response.data as { success?: boolean; status?: string; error?: unknown; message?: string };
      ...
      if (responseBody.success === false || status === "error" || status === "failed") {
        throw new Error(
          responseBody.message ?? `AI training API reported failure: ${JSON.stringify(responseBody.error ?? responseBody)}`
        );
      }
...
export async function scheduleAiTrainingCronJob() {
  await aiTrainingQueue.upsertJobScheduler(
